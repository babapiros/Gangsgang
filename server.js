...
app.post("/kick-user", async (req, res) => {
    ...
    try {
        const guild = client.guilds.cache.get("1235189205571866655");
        const member = await guild.members.fetch(memberId);

        if (member) {
            await member.kick("Admin tarafından sunucudan atıldı.");
            return res.json({ success: true, message: `${member.user.username} sunucudan atıldı.` });
        }

        return res.json({ success: false, message: "Kullanıcı bulunamadı." });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: "Kullanıcıyı atarken bir hata oluştu." });
    }
});
...
app.post("/ban-user", async (req, res) => {
    ...
    try {
        ...
        if (member) {
            await member.ban({ reason: "Admin tarafından yasaklandı." });
            return res.json({ success: true, message: `${member.user.username} sunucudan yasaklandı.` });
        }

        return res.json({ success: false, message: "Kullanıcı bulunamadı." });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: "Kullanıcıyı yasaklarken bir hata oluştu." });
    }
});
